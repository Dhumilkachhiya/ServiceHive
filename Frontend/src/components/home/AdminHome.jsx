...
export default function AdminHome() {
    ...
    const fetchAllData = async () => {
        try {
            const [statsRes, usersRes, compRes, reqRes] = await Promise.all([
    ...
    const handleVerify = async (id) => {
        try {
            const response = await api.patch(`/api/admin/companies/${id}/verify`, {});
            alert("Success: " + response.data.message);
            fetchAllData();
        } catch (err) {
            alert("Error verifying company: " + (err.response?.data?.message || err.message));
            console.error(err);
        }
    };
